...
public class RestaurantHealthCheck implements HealthIndicator {
    ...
    public Health health() {
        var count = restaurantRepository.getRestaurantCount();
        return count > 0 ? Health.up().build() : Health.down().withDetail("count", count).status(new Status("NO_RESTAURANTS")).build();
    }
